...
### 七、结构化用药清单（JSON）
在报告的末尾，请附加一个 JSON 代码块，包含所有建议的药物信息，用于自动填入电子处方系统。格式如下：
...
```json
{
  ...
  "medications": [
    {
      "drug_name": "药品通用名（中文）",
      "specification": "规格（如 5mg/片、0.25g/粒）",
      "dosage": "单次用量（如 1片、2粒、10ml）",
      "usage_method": "用法（口服/静脉注射/肌内注射/皮下注射/外用/舌下含服/雾化吸入）",
      "frequency": "频次（每日1次/每日2次/每日3次/每日4次/必要时/睡前）",
      "days": 7
    }
  ],
  "notes": "用药注意事项（如饭后服用、避免饮酒、定期监测肝肾功能等）"
}
```

要求：
- drug_name 使用中文通用名
- specification 要包含单位和剂型
- usage_method 从列举的选项中选择
- frequency 从列举的选项中选择
- days 为整数天数
- 仅输出指南或历史病例中有明确依据的药物，宁缺毋滥
"""
